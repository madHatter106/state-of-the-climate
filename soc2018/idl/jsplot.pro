...
;-------------------------------------------------------------
	...
	pro jsplot, t, y0, help=hlp, charsize=csz, color=clr, $
	  ...
	  sun=sun, long=lng, lat=lat, zone=zone, xticklabelstep=xticklabelstep
...
;	if n_elements(cclr) eq 0 then cclr=clr
	...
	if n_elements(symsize) eq 0 then symsize=!p.symsize
        if n_elements(xticklabelstep) eq 0 then xticklabelstep = 1
	...
	if keyword_set(sun) then begin
	  ...
	  if !d.n_colors lt 181 then begin
            print,' Error in jsplot: Only have '+strtrim(!d.n_colors,2)
	...
	if not keyword_set(over) then $
	  timeaxis, jd=jd, form=frm, nticks=ntics, title=xtitl, $
	    ...
	    color=clr, major=mjr, minor=mnr, xticklabelstep=xticklabelstep
